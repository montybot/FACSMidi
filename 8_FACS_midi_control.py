...
class MidiThread(QThread):
    ...
    def __init__(self, intensity_slider, msg_box, modifiers_sliders):
        QThread.__init__(self, None)
        self.exiting = False
        self.intensity_slider = intensity_slider
        self.msg_box          = msg_box
        self.sliders          = modifiers_sliders
        log.message('letype %s', self.sliders)
        self.intensity        = 0
        self.AU_to_mix = [
        ["01_Langner_Angry", False],
        ["02_Langner_Contemptuous", False],
        ["03_Langner_Disgusted", False],
        ["05_Langner_Fearfull", False],
        ["06_Langner_Happy", False],
        ["06_Langner_Happy_A", False],
        ["06_Langner_Happy_B", False],
        ["07_Langner_Sadness", False],
        ["08_Langner_Surprised", False],
        ["08_Surprise", False],
        ["02_Contempt", False],
        ["09_Pride", False],
        ["06_Happy", False],
        ...
        #self.hm = G.app.selectedHuman
        #self.emotions = G.app.selectedHuman.getModifiersByGroup('Emotions Blender')
        #self.test = self.emotions[0]
        #log.message('letype %s', self.test)

        self.API = rtmidi.API_WINDOWS_MM
        self.midi_in = rtmidi.MidiIn(rtapi=self.API, name="FACSHuman")
        
        log.message('FACSmidi START')
        self.port = 0 #sys.argv[1] if len(sys.argv) > 1 else None
        
    def run(self):
        self.le_switch = False
        try:
            self.midiin = self.midi_in.open_port(0)
        except (EOFError):
            pass #sys.exit()
        try:
            while not self.exiting:
               ...
               #log.message('letype %s', type(self.intensity))
               #First line of pad
                   if (self.msg[0][1] == 48) and (self.msg[0][2] == 100):
                      self.AU_to_mix[0][1] = True
                   if (self.msg[0][1] == 48) and (self.msg[0][2] == 0):
                      self.AU_to_mix[0][1] = False
                      
                   if (self.msg[0][1] == 49) and (self.msg[0][2] == 100):
                      self.AU_to_mix[1][1] = True
                   if (self.msg[0][1] == 49) and (self.msg[0][2] == 0):
                      self.AU_to_mix[1][1] = False
                      
                   if (self.msg[0][1] == 50) and (self.msg[0][2] == 100):
                      self.AU_to_mix[2][1] = True
                   if (self.msg[0][1] == 50) and (self.msg[0][2] == 0):
                      self.AU_to_mix[2][1] = False

                   if (self.msg[0][1] == 51) and (self.msg[0][2] == 100):
                      self.AU_to_mix[3][1] = True
                   if (self.msg[0][1] == 51) and (self.msg[0][2] == 0):
                      self.AU_to_mix[3][1] = False

               #First line of pad
                   if (self.msg[0][1] == 44) and (self.msg[0][2] == 100):
                      self.AU_to_mix[4][1] = True
                   if (self.msg[0][1] == 44) and (self.msg[0][2] == 0):
                      self.AU_to_mix[4][1] = False
                      
                   if (self.msg[0][1] == 45) and (self.msg[0][2] == 100):
                      self.AU_to_mix[5][1] = True
                   if (self.msg[0][1] == 45) and (self.msg[0][2] == 0):
                      self.AU_to_mix[5][1] = False
                      
                   if (self.msg[0][1] == 46) and (self.msg[0][2] == 100):
                      self.AU_to_mix[6][1] = True
                   if (self.msg[0][1] == 46) and (self.msg[0][2] == 0):
                      self.AU_to_mix[6][1] = False

                   if (self.msg[0][1] == 47) and (self.msg[0][2] == 100):
                      self.AU_to_mix[7][1] = True
                   if (self.msg[0][1] == 47) and (self.msg[0][2] == 0):
                      self.AU_to_mix[7][1] = False

               #First line of pad
                   if (self.msg[0][1] == 40) and (self.msg[0][2] == 100):
                      self.AU_to_mix[8][1] = True
                   if (self.msg[0][1] == 40) and (self.msg[0][2] == 0):
                      self.AU_to_mix[8][1] = False
                      
                   if (self.msg[0][1] == 41) and (self.msg[0][2] == 100):
                      self.AU_to_mix[9][1] = True
                   if (self.msg[0][1] == 41) and (self.msg[0][2] == 0):
                      self.AU_to_mix[9][1] = False
                      
                   if (self.msg[0][1] == 42) and (self.msg[0][2] == 100):
                      self.AU_to_mix[10][1] = True
                   if (self.msg[0][1] == 42) and (self.msg[0][2] == 0):
                      self.AU_to_mix[10][1] = False

                   if (self.msg[0][1] == 43) and (self.msg[0][2] == 100):
                      self.AU_to_mix[11][1] = True
                   if (self.msg[0][1] == 43) and (self.msg[0][2] == 0):
                      self.AU_to_mix[11][1] = False

               #First line of pad
                   if (self.msg[0][1] == 36) and (self.msg[0][2] == 100):
                      self.AU_to_mix[12][1] = True
                   if (self.msg[0][1] == 36) and (self.msg[0][2] == 0):
                      self.AU_to_mix[12][1] = False
                      
                   if (self.msg[0][1] == 37) and (self.msg[0][2] == 100):
                      self.AU_to_mix[13][1] = True
                   if (self.msg[0][1] == 37) and (self.msg[0][2] == 0):
                      self.AU_to_mix[13][1] = False
                      
                   if (self.msg[0][1] == 38) and (self.msg[0][2] == 100):
                      self.AU_to_mix[14][1] = True
                   if (self.msg[0][1] == 38) and (self.msg[0][2] == 0):
                      self.AU_to_mix[14][1] = False

                   if (self.msg[0][1] == 39) and (self.msg[0][2] == 100):
                      self.AU_to_mix[15][1] = True
                   if (self.msg[0][1] == 39) and (self.msg[0][2] == 0):
                      self.AU_to_mix[15][1] = False
                      

                   if self.msg[0][1] == 74:
                      self.intensity = self.msg[0][2]
                      self.intensity_slider.onChanging(self.intensity)
                      self.intensity_slider.setValue(self.intensity)
                      if self.AU_to_mix[0][1] == True:
                         self.sliders[self.AU_to_mix[0][0]].onChanging(round(float(self.intensity)/127,2))
                      if self.AU_to_mix[1][1] == True:
                         self.sliders[self.AU_to_mix[1][0]].onChanging(round(float(self.intensity)/127,2))
                      if self.AU_to_mix[2][1] == True:
                         self.sliders[self.AU_to_mix[2][0]].onChanging(round(float(self.intensity)/127,2))
                      if self.AU_to_mix[3][1] == True:
                         self.sliders[self.AU_to_mix[3][0]].onChanging(round(float(self.intensity)/127,2))
                      if self.AU_to_mix[4][1] == True:
                         self.sliders[self.AU_to_mix[4][0]].onChanging(round(float(self.intensity)/127,2))
                      if self.AU_to_mix[5][1] == True:
                         self.sliders[self.AU_to_mix[5][0]].onChanging(round(float(self.intensity)/127,2))
                      if self.AU_to_mix[6][1] == True:
                         self.sliders[self.AU_to_mix[6][0]].onChanging(round(float(self.intensity)/127,2))
                      if self.AU_to_mix[7][1] == True:
                         self.sliders[self.AU_to_mix[7][0]].onChanging(round(float(self.intensity)/127,2))
                      if self.AU_to_mix[8][1] == True:
                         self.sliders[self.AU_to_mix[8][0]].onChanging(round(float(self.intensity)/127,2))
                      if self.AU_to_mix[9][1] == True:
                         self.sliders[self.AU_to_mix[9][0]].onChanging(round(float(self.intensity)/127,2))
                      if self.AU_to_mix[10][1] == True:
                         self.sliders[self.AU_to_mix[10][0]].onChanging(round(float(self.intensity)/127,2))
                      if self.AU_to_mix[11][1] == True:
                         self.sliders[self.AU_to_mix[11][0]].onChanging(round(float(self.intensity)/127,2))
                      if self.AU_to_mix[12][1] == True:
                         self.sliders[self.AU_to_mix[12][0]].onChanging(round(float(self.intensity)/127,2))
                      if self.AU_to_mix[13][1] == True:
                         self.sliders[self.AU_to_mix[13][0]].onChanging(round(float(self.intensity)/127,2))
                      if self.AU_to_mix[14][1] == True:
                         self.sliders[self.AU_to_mix[14][0]].onChanging(round(float(self.intensity)/127,2))
                      if self.AU_to_mix[15][1] == True:
                         self.sliders[self.AU_to_mix[15][0]].onChanging(round(float(self.intensity)/127,2))
                   
                   
                #   if self.msg[0][1] == 48 & self.msg[0][2] == 100:
                #      if self.msg[0][1] == 71:
                #         self.intensity = self.msg[0][2]
                #         self.intensity_slider.onChanging(self.intensity)
                #         self.intensity_slider.setValue(self.intensity)
                #      self.sliders['01_Langner_Angry'].onChanging(self.intensity)
                #
                #   if self.msg[0][1] == 48 & self.msg[0][2] == 0:
                #      self.sliders['01_Langner_Angry'].onChanging(0)
      
                #   if self.msg[0][1] == 49:
                #      self.sliders['08_Surprise'].onChanging(self.msg[0][2])
                #       #log.message('letype %s', round(self.AU_intensity,2))
                #   #self.msg_box.setText(str(round(self.AU_intensity,2)))
                       #self.intensity_slider.update()
                       
                       #self.test.setValue(round(self.AU_intensity,2))
                       #theslider(self.AU_intensity)
               self.usleep(500)
    ...
    def slider_assign(self, theslider):
        intensity_slider = theslider
...
class FACSTest(gui3d.TaskView, object):
    ...
    def __init__(self, category, appFacsAnim):
        gui3d.TaskView.__init__(self, category, 'FACS Midi Controller')
        ...
        box_midi_list = self.addLeftWidget(gui.GroupBox('Midi List ports chooser'))
...
# Box tools        
        ...
        # def onChange(value):
            # anim_task_view.setOneAuTimelineValue(au_timeline, au_event, 'intensity_stop', value)
# Box tools        
        self.animation_test = box_aus_list.addWidget(gui.Slider(value=0.00, min=0.00, max=127, label=['Animation frame test : ','%d']), columnSpan = 3)
        self.txt_number_of_frame = box_aus_list.addWidget(gui.TextView('Frames : '), 1, 0)
        self.images_number_to_render = box_aus_list.addWidget(gui.TextEdit(text='127'), 1, 1)
...
#Box Midi
        self.midi_list_items = box_midi_list.addWidget(gui.ListView(), columnSpan = 3)
        self.midi_msg_received = box_midi_list.addWidget(gui.TextEdit(text='25'), columnSpan = 3)
        self.midi_start_stop = box_midi_list.addWidget(gui.CheckBox('Midi'))
        self.midi_label = box_midi_list.addWidget(gui.TextView('STOP'))
...
# Midi Thread creation
        self.facs_midi = MidiThread(self.animation_test, self.midi_msg_received, self.modifiers_sliders)

        @self.midi_start_stop.mhEvent
        def onClicked(event):
            if self.midi_start_stop.selected:
                self.midi_label.setText('START')
                #self.facs_midi.slider_assign(self.animation_test)
                #self.facs_midi.text_box_assign(self.midi_msg_received)
                self.facs_midi.start()
                #self.facs_midi.startListening()
            else:
                self.midi_label.setText('STOP')
                self.facs_midi.stopListening()
...
##########################################################################
        ...
        def onChange(value):
            try:
                ...
                self.animation_test.setMax(127)
...
# 
##########################################################################
#    facsmidi_thread = facs_midi
    #Thread MIDI
    

    def midi_to_FACSHuman_start(self, facs_midi):
        log.message('toto = %s', type(facs_midi))
        #self.facs_midi.msgtxtbox(self.midi_msg_received)
        # if not facsmidi_thread is None:
        #    facsmidi_thread.start()
        pass

    def midi_to_FACSHuman_stop(self, facs_midi):
        #if not facsmidi_thread is None:
        #    facsmidi_thread.stopListening();
        pass
        
    def set_slider_sequencer(self, value):
        self.slidersSequencerRenderImageSet(value)
